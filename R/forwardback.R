...
forwardback <- function(probTran, initQ) {
    ...
    ## probTran is an nObs x nQ^2 matrix with: each row of probTran reprsents P(Obs_k
    ## | state_k ) * P(state_k | state_{k-1}) ProbTran is a row-wise matrix such that
    ## matrix(probTran[1,], ncol=nQ, nrow=nQ, byrow =TRUE) is the probability above in
    ## matrix format returns a list of alpha = forward prob, beta = backward prob,
    ## such that normalize(alpha[i] *beta[i]) is the smoothed HMM state estimate alpha
    ## and beta are row-wise matrix vectors
