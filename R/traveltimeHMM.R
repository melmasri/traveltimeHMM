...
traveltimeHMM<-function(logspeeds, trips, timeBins, linkIds, nQ=1, model = c('HMM','trip-HMM','no-dependence'), tolErr=1, L=10, maxItra=NULL, verbose=TRUE){
    ## ##################################################
    ...
    indexLinksLessMinObs= sapply(gsub("[0-9.]+", "", names(linksLessMinObs)), function(r) which(r == levels(timeFactor)))
    ...
    nInitalObsPerFactor = sapply(split(1:nlevels(tripsFactor), linkTimeFactor[initialObsIds]), length)
    ...
    indexInitialObsLessMinObs= sapply(gsub("[0-9.]+", "", names(initialObsLessMinObs)), function(r) which(r == levels(timeFactor)))
    
    ## finding the number of factors (link x timeBin) that have only initial states, i.e cannot compute P(state_{k-1}, state_k | Obs})
    factorsWithOnlyInitialObs = which(sapply(split(1:nObs, linkTimeFactor), length) == nInitalObsPerFactor)
    indexFactorsWithOnlyInitialObs = sapply(gsub("[0-9.]+", "", names(factorsWithOnlyInitialObs)), function(r) which(r == levels(timeFactor)))

    ## ##################################################
    ## Storage variables
    ## Gaussian parameter matrices
    ...
    error <-function(A, B) if(is.null(A) || is.null(B)) 0 else sqrt(sum(abs(A-B)^2))
    ## ##################################################
    ## Start of simulation
    ...
    repeat{
        ## forward-backward
        if(grepl('HMM', model)){
            probTran = tmat[paramId, ] * pmax(dnorm(logspeeds,  meanGauss[paramId, ], varGauss[paramId, ]), 1e-3 )[,rep(1:nQ, nQ)]
            fb =  tapply(1:nObs, tripsFactor, function(r) forwardback(probTran[r,], init[paramId[r[1]],]))
            
            ## probability of each state per observation
            alpha = matrix(unlist(lapply(fb, function(r) r$alpha),use.names=FALSE), ncol=nQ, byrow=TRUE)  # forwad prob. normalized
            beta  = matrix(unlist(lapply(fb, function(r) r$beta), use.names=FALSE), ncol=nQ, byrow=TRUE)  # backward prob. normalized
            probStates = normalizeR(alpha*beta + 1e-5, m = nObs, n = nQ)                                         # smoothed and normalized prob
            
            ## calculating initial state probability
            initNew = posterior_initial_states(probStates, linkTimeFactor, initialObsIds)
            if(!is.null(indexInitialObsLessMinObs)){
                initAverage = posterior_initial_states(probStates, timeFactor, initialObsIds)
                initNew[initialObsLessMinObs,] = initAverage[indexInitialObsLessMinObs,]
            }
            ## calculating conditional joint transition probability
            ## P(state_k, state_{k-1} | Obs)
            probJointStates = normalizeR(alpha[-nObs,rep(1:nQ, each=nQ)] * (probTran * beta[,rep(1:nQ, nQ)]) [-1,], m = nObs -1, nQ2)
            probJointStates[initialObsIds[-1]-1, ]<-0           # setting the overlapping multiplication between trips to 0
            
            numGamma = lapply(split(probJointStates, linkTimeFactor[-1]), function(r) .colSums(r, length(r)/nQ2, nQ2))
            numGamma = matrix(unlist(numGamma, use.names=FALSE), ncol = nQ2, byrow=TRUE)
            denGamma = probStates
            denGamma[initialObsIds[-1]-1,]<-0
            denGamma = lapply(split(denGamma[-nObs,], linkTimeFactor[-1]), function(r) .colSums(r, length(r)/nQ, nQ))
            denGamma = matrix(unlist(denGamma, use.names=FALSE), ncol=nQ, byrow=TRUE)
            tmatNew = numGamma/denGamma[, rep(1:nQ, each = nQ)]
            
            if(!is.null(indexInitialObsLessMinObs)){
                numGamma = lapply(split(probJointStates, timeFactor[-1]), function(r)  .colSums(r, length(r)/nQ2, nQ2))
                numGamma = matrix(unlist(numGamma, use.names=FALSE), ncol = nQ2, byrow=TRUE)
                denGamma = probStates
                denGamma[initialObsIds[-1]-1,]<-0
                denGamma = lapply(split(denGamma[-nObs,], timeFactor[-1]), function(r) .colSums(r, length(r)/nQ, nQ))
                denGamma = matrix(unlist(denGamma, use.names=FALSE), ncol=nQ, byrow=TRUE)
                tmatAverage = numGamma/denGamma[, rep(1:nQ, each = nQ)]
                tmatNew[initialObsLessMinObs, ] <- tmatAverage[indexInitialObsLessMinObs,] # applying to all factors with less than min Obs
        ...
        if(!is.null(ord$order)){
            meanGaussNew[ord$toSort,] = t(sapply(1:length(ord$toSort), function(r) meanGaussNew[ord$toSort[r], ord$order[r,] ], USE.NAMES=FALSE))
            varGaussNew[ord$toSort,] = t(sapply(1:length(ord$toSort), function(r) varGaussNew[ord$toSort[r], ord$order[r,] ], USE.NAMES=FALSE))
        }

        ## Calculating || ThetaNew - Theta ||
        maxError = sum(error(init, initNew),  error(tmat, tmatNew), error(meanGauss, meanGaussNew),  error(varGaussNew, varGauss))
        ...
        iter = iter + 1
        print(paste(round(maxError,3), 'total error in iteration', iter, ' @', format(Sys.time()- tstart)))
    ...
    ## rownames(tmat)<-rownames(init)<-rownames(varGauss)<-rownames(meanGauss)<- levels(linkTimeFactor)
    ## rownames(tmatAverage)<-rownames(initAverage)<-rownames( meanSigAverage$mean)<-rownames(meanSigAverage$sigma)<-levels(timeFactor)
    
    ## returning variabels
    invisible(list(factors = linkTimeFactor, tmat = tmat, init = init, sd = sqrt(varGauss), mean = meanGauss, nQ = nQ, nB = nB, nObs  = nObs, model = model,
                   tmatAverage= tmatAverage,  initAverage = initAverage, meanAverage = meanSigAverage$mean, sdAverage = sqrt(meanSigAverage$sigma)))
}
