...
The estimation algorithm can be executed by calling the `traveltimeHMM` function.  The function's interface is as follows. We will explore its parameters in the next few sections.
...
In the end, predictions can be obtained by simulation on the basis of these parameter estimates.  In our example, we obtain 1 000 independent predictions of the total travel speed on the selected trip using as many simulations.# Where to put?  ÉG 2019/07/22
