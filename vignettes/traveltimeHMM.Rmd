...
The other three model types are special cases of the most general model.  The "HMM" model type, which is the default type, makes abstraction of the trip effect: $\log(E_i)$ is restricted to $0$ whilst $\tau$ is set to $1$^[$\tau$ should also be set to $0$ instead of $1$.  ÉG 2019/07/25].  The "trip" model type instead considers the existence of a single congestion state; under this model the parameters $\gamma$ and $\Gamma$ remain undefined.  Finally, the "no-dependence" model type, which the most basic of the four, combines the restrictions of the "HMM" and "trip" model types and only takes into account parameters $\mu_{j,b,q}$ and $\sigma_{j,b,q}$ for the speed probability distribution.  Table 2 provides a summary of the characteristics of each model type.
